...
def run(numberOfGenerations, populationSize, crossoverFactor, mutationFactor, input1, input2, outputFile):
    p = Population(populationSize, input1, input2)
    p.evaluateAll()
    for i in range(0, numberOfGenerations):

        ##crossover
        for j in range (0, int(crossoverFactor*populationSize)):                              
            p.crossover(int(random.uniform(0, populationSize)), int(random.uniform(0, populationSize)))

        ##mutation
        for j in range (0, int(mutationFactor*populationSize)):    
            p.mutation(int(random.uniform(0, populationSize)))
...
def main():
    ...
    parser.add_argument('-o','--output',help='Output file name', type=str, default='..\..\data\outputSimilitudes.txt',  required=False)
    parser.add_argument('-nGen','--nbGenerations',help='Number Of Generations', type=int,  required=False)
